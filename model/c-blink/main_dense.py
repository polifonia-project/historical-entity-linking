...
def _run_biencoder(biencoder, dataloader, candidate_encoding, constraint_dict, top_k=100, indexer=None):
    ...
    for batch in tqdm(dataloader):
        ...
        #meta_for_mhercl
        meta = [{"type": [data["type"][0][sample_idx], data["type"][1][sample_idx], data["type"][2][sample_idx]],  "date": data["date"][sample_idx].item() }
                 for sample_idx in range(label_ids.shape[0])]
        #meta for clef-hipe
        #meta = [{"type": [data["type"][0][sample_idx]], "date": data["date"][sample_idx].item() } for sample_idx in range(label_ids.shape[0])]
        ...
        with torch.no_grad():
            if indexer is not None:
                ...
                scores = biencoder.score_candidate(
                    context_input, meta, None, cand_encs=candidate_encoding, constraint_dict=constraint_dict  # .to(device)
                )
...
def load_models(args, logger=None):
    ...
    # biencoder_params["projection_path"] = args.projection_path
...
def run(
    ...
    with open(args.constraint_dict_path, "rb") as f:
        constraint_dict = pickle.load(f)
    ...
    while not stopping_condition:
        ...
        labels, nns, scores = _run_biencoder(
            biencoder, dataloader, candidate_encoding, constraint_dict, top_k, faiss_indexer
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        help="Path to the projection layer for projecting the biencoder output.",
    )
    parser.add_argument(
        "--biencoder_config",
